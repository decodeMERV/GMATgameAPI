...
const QUESTION_FIELDS = ['id','question', 'answerA', 'answerB', 'answerC', 'answerD', 'answerE', 'level', 'correctAnswer'];
...
//this method gets the next question from the DB depending on if the user answers the question correctly or not
//NEED TO VERIFY IF THIS WORKS OR NOT

  getNextQuestion(currentQuestionLevel, isCorrect) {
    ...
    if(isCorrect) {
      return this.query(
        ...
        .select('QUESTION_FIELDS')
          .from('questions')
          .where({
            'questions.level': currentQuestionLevel + 100
          })
          .orderBy('RAND()')
          .limit(1)
      )
...
*/
      ...
      return this.query(
        ...
        .select('QUESTION_FIELDS')
          .from('questions')
          .where({
            'questions.level': currentQuestionLevel
        })
      )
    }
  }
