...
//This method retrieves the next question from the DB depending on the user's response
  ...
  insertQuestion(questionInsObj) {
    var query = knex
      .insert({
        ...
        categoryId: questionInsObj.category,
  ...
  getLeaders(){
    ...
    return this.query(
      ...
      knex
        .select('username as User',knex.raw('SUM(score)'))
        .from('leader_board')
        .groupBy('username')
        .orderByRaw('2 DESC')
        .limit(10)
  ...
  getProfileUpdate(userInfo, sessionToken){
    console.log(sessionToken,'MMMMMMMMMMMMMM')
    console.log(userInfo, 'HAHAHAHAH')
    var filtered_info= util.filterKeys(USER_UPDATE_FIELDS, userInfo);
    console.log(filtered_info)
    console.log('fields: ', USER_UPDATE_FIELDS, "info: ", userInfo)
    return this.query(
      knex('users')
