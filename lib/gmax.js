...
//This method retrieves the next question from the DB depending on the user's response
  ...
  getLeaders(){
    return this.query(
      knex
        .select('leader_board.username as user',knex.raw('SUM(leader_board.score) as total'),knex.raw('COUNT(leader_board.username) as cnt'),knex.raw('SUM(leader_board.time)/60000 as time'),'users.email as email')
        .from('leader_board')
