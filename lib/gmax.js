  ...
  getUserFromSession(sessionToken) {
    return this.query(
      knex
       ...
       .select('users.id as id', 'users.email as email', 'users.createdAt as createdAt', 'users.updatedAt as updatedAt','users.username as username','users.interests as interests')
        .from('sessions')
...
//This method retrieves the next question from the DB depending on the user's response
  ...
  getNextQuestion(currentLevel, isCorrect) {
    ...
    var query = (
      knex
        .select('questions.id as id', 'questions.title as title', 'questions.answerA as answerA', 'questions.answerB as answerB', 'questions.answerC as answerC', 'questions.answerD as answerD', 'questions.answerE as answerE', 'questions.categoryId as category', 'questions.level as level', 'questions.correctAnswer as correctAnswer', 'category.name as category')
        .from('questions')
...
gameEngine class
  ...
  5. getNextQuestion: fetches the next question from the DB
--> for getQuestion, we can select the queries but if we want to render them into different components, how do we set it up? Do we create separate methods to display each box?
  ...
  1. getTimer
  2. getCurrentScore: gets the score from the user table and updates it retroactively
  3. getCurrent
  4. getAnimation: fetches animation of astronaut
